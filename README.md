...
- **Adaptive coding execution:** the main thread keeps decision, integration, and final-acceptance ownership. Small or tightly coupled work may stay local; independent high-volume execution may use short-lived Subagents with bounded context and evidence requirements. Delegation is selected only when it creates net benefit.
- **Subagent Result Capsules:** repo-local `SubagentStart` and `SubagentStop` hooks require workers to return compact `ACTION`, actual `VERIFY`, optional `ASSESS`, and an artifact pointer. Worker `DONE` remains separate from root acceptance, and rework uses explicit revisions so an older result cannot remain canonical.
...
- **Open-source reuse route:** every detailed Goal is authored after current tool/article research. Goal mode records where accepted tools are used and how they are validated, then requires a project-level refresh every 24 hours of continued work. Continuations within the window reuse the cached result.
- **Segment deadlines:** every capability segment has an hour-level target. A first product write silently starts the only unambiguous dependency-ready segment; ambiguous parallel segments require explicit selection. Starting creates a real wall-clock deadline, and segments longer than two hours use a Goal-selected reminder cadence until evidence-backed completion. Reminders occur on subsequent Codex events and do not claim they can wake an idle client.
- **Super-complex plan:** project-relative Markdown/README over 4,000 characters, referenced by a real 2,000-3,500 character Goal-mode contract. The Agent researches existing tools and articles first; if reuse is viable, it visibly asks whether to use or adapt it. Commercial status is requested only when it changes license compatibility, pricing, permitted use, or the integration boundary.
...
- **Coding execution policy:** a detailed installed protocol chooses direct main-thread work or bounded delegation by expected context/time savings, task coupling, risk, and independent verifiability. `Luna Max` is the preferred high-volume coding worker when compatible, but model choice is not evidence and does not override user or role-specific routing.
- **Subagent context governance:** Subagents receive only their bounded assignment and return one short Result Capsule. Detailed logs and explanations remain in the workspace or local artifacts; `status --verbose` exposes the revision index for pull-on-demand inspection. The current Codex Hook API can require a corrected child final but cannot replace parent-visible transport payloads, so the plugin records exhausted violations as unverified instead of claiming a transport guarantee.
...
- **Route convergence:** every technical route remains anchored to the detailed Goal's source requirements, first principles, and final acceptance. Repeated same-cause failures without new evidence trigger a low-cost route reassessment and online reuse search; only another exact retry can reach a high-confidence, action-scoped rail, while materially different routes stay available.
