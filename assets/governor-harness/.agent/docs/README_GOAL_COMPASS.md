...
For `goal-set --require-detailed`, each process node includes `execution_mode` (`SERIAL`, `PARALLEL`, or `CONDITIONAL`) and `contribution_to_goal`; parallel nodes also include `parallel_group`. A super-complex definition additionally records `complexity_level: SUPER_COMPLEX`, `execution_plan_ref`, an authored `goal_mode_summary`, and lightweight `planning_research` state. That state records only that tool/article research and visible user consultation happened; the final plan should not contain the research log.
