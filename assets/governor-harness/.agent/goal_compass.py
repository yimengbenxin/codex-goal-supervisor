...
def cmd_feedback_config(args: argparse.Namespace) -> int:
    try:
        config = configure_feedback(
            agent_dir=AGENT,
            endpoint=args.endpoint,
            token_env=args.token_env,
            capture_enabled=False if args.disable else True if args.enable else None,
            timeout_seconds=args.timeout,
            deployment_context=args.context,
            upload_enabled=True if args.allow_upload else False if args.deny_upload else None,
            confirm_upload=args.confirm_upload,
        )
    except ValueError as exc:
        print(json.dumps({"ok": False, "error": str(exc)}, ensure_ascii=False, indent=2))
        return 2
    # Explicit upload consent is also the zero-configuration device bootstrap.
    # No registration or network call occurs for the default/denied path.
    delivery = flush_feedback(agent_dir=AGENT, max_events=25, force=True) if args.flush or args.allow_upload else None
    current_status = feedback_status(AGENT)
    upload_ready = bool(current_status.get("upload_ready"))
    explicit_upload_not_ready = bool(args.allow_upload and not upload_ready)
    print(json.dumps({
        "ok": not explicit_upload_not_ready,
        "capture_enabled": config.get("capture_enabled"),
        "deployment_context": config.get("deployment_context"),
        "privacy_choice_required": current_status.get("privacy_choice_required"),
        "upload_authorized": feedback_upload_authorized(config),
        "remote_configured": current_status.get("remote_configured"),
        "credentials_configured": current_status.get("credentials_configured"),
        "upload_ready": upload_ready,
        "delivery_status": current_status.get("delivery_status"),
        "delivery_mode": config.get("delivery"),
        "privacy_mode": config.get("privacy_mode"),
        "delivery": delivery,
        "required_action": "retry_automatic_device_registration" if explicit_upload_not_ready else None,
    }, ensure_ascii=False, indent=2))
    return 2 if explicit_upload_not_ready else 0


def cmd_feedback(args: argparse.Namespace) -> int:
    if args.flush:
        result = flush_feedback(agent_dir=AGENT, max_events=100, force=True)
        current_status = feedback_status(AGENT)
        authorized = bool(current_status.get("upload_authorized"))
        ok = not authorized or (int(result.get("pending", 0) or 0) == 0 and not result.get("last_error"))
        print(json.dumps({
            "ok": ok,
            "uploaded": bool(authorized and ok),
            "queued_locally": int(result.get("pending", 0) or 0) > 0,
            "delivery": result,
            "required_action": None if ok else "configure_device_or_retry_upload",
        }, ensure_ascii=False, indent=2))
        return 0 if ok else 2
    if not args.kind or not args.message:
        print(json.dumps({"ok": False, "error": "--kind and --message are required unless --flush is used"}, ensure_ascii=False))
        return 2
    ...
    ok = bool(result.get("captured")) and (not authorized or uploaded)
    print(json.dumps({
        "ok": ok,
        **result,
        "required_action": None if ok else "configure_device_or_retry_upload",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Compatibility-only deployment overrides. Normal users never configure
    # endpoints or credentials; explicit consent triggers automatic bootstrap.
    p.add_argument("--endpoint", help=argparse.SUPPRESS)
    p.add_argument("--token-env", help=argparse.SUPPRESS)
    p.add_argument("--timeout", type=float, help=argparse.SUPPRESS)
    p.add_argument("--context", choices=["unknown", "enterprise", "personal"])
    ...
    upload.add_argument("--deny-upload", action="store_true")
    p.add_argument("--confirm-upload", action="store_true")
    p.add_argument("--flush", action="store_true")
    p.set_defaults(func=cmd_feedback_config)
