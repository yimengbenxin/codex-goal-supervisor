...
def prompt_yes_no(message: str) -> bool | None:
    try:
        return input(message).strip().lower() in {"y", "yes", "是"}
    except EOFError:
        # Some Windows CI and redirected shells report a TTY but provide no
        # readable input. Preserve the privacy-safe default instead of failing.
        return None


def resolve_feedback_policy(args: argparse.Namespace) -> tuple[str, bool]:
    context = args.feedback_context
    allow_upload = bool(args.allow_feedback_upload)
    explicit_upload_choice = bool(args.allow_feedback_upload or args.deny_feedback_upload)

    if args.allow_feedback_upload and not args.confirm_feedback_upload:
        raise SystemExit("--allow-feedback-upload requires --confirm-feedback-upload")
    if args.allow_feedback_upload and context not in {"enterprise", "personal"}:
        raise SystemExit("--allow-feedback-upload requires --feedback-context enterprise or personal")

    if sys.stdin.isatty():
        if context is None:
            enterprise = prompt_yes_no("是否为企业/工作场景？默认按非企业处理 [y/N] ")
            if enterprise is not None:
                context = "enterprise" if enterprise else "personal"
        if not explicit_upload_choice:
            allow_upload = bool(prompt_yes_no("是否允许此项目上传经过脱敏的治理反馈？默认不上传 [y/N] "))
    else:
        context = context or "unknown"
        allow_upload = allow_upload if explicit_upload_choice else False
    return context or "unknown", allow_upload


def configure_feedback_policy(target: Path, context: str, allow_upload: bool) -> int:
    compass = target / ".agent" / "goal_compass.py"
    if not compass.exists():
        print("goal_compass.py missing before feedback policy setup", file=sys.stderr)
        return 2
    command = [
        sys.executable,
        str(compass),
    ...
    if result.returncode != 0:
        print(result.stderr or result.stdout or "feedback policy setup failed", file=sys.stderr)
