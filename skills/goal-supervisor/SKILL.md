...
- If a confirmed North Star already exists, preserve it exactly and reuse its stored `goal_mode_objective` as the native Goal payload. If no confirmed North Star exists, build the detailed contract and run `goal-set --require-detailed`; its app-server bridge installs and verifies that returned payload. Never claim activation is complete when only one of these two states exists or their objective text differs.
...
- Feedback upload is disabled by default. When `privacy_choice_required` is true, ask one concise question in the user's language about work/enterprise context and redacted diagnostic upload. Do not ask the user to run a command; apply the answer internally. This is a one-time project choice. The user may answer the upload part clearly; otherwise choose no upload.
...
Technical-route control is domain-neutral. Every route must be judged against the detailed Goal's source requirements, first principles, and final acceptance, not merely whether one local step can run. A development-only route may remain as a test lane, but it cannot replace a required production usage path. The background observer groups failures by the active Goal route and a redacted cause family. One occurrence stays silent. Two occurrences separated by 30 minutes, or three immediate occurrences without new Goal evidence, require route reassessment: research current tools and documented solutions online, compare at least two materially different routes, and execute the smallest route that can satisfy the same acceptance. Continued repetition moves only the already-proven failed action behind a sparse semantic rail; unrelated work and genuinely different route trials remain available. Do not reset the incident because an unrelated command succeeded.
...
- Feedback is local-only by default. The plugin performs no device registration and no network request until the current project has explicit upload consent.
- "Record this plugin problem" means durable local capture only.
- A clear instruction such as "upload/sync these plugin problems to my feedback server" is explicit consent for the current project. If the work/enterprise context is still unknown, ask one concise privacy question; otherwise run `feedback-config --context <personal|enterprise> --allow-upload --confirm-upload --flush` for the user. This automatically registers the device and stores its revocable credential privately; never ask the user for an endpoint or Token.
- Do not ask a non-technical user to type the CLI command. Execute it after consent and report only whether delivery succeeded or remains queued locally.
...
4. Run `phase-set --outline-file <outline.json> --definition-file <phase.json>`.
   The command installs the exact returned `goal_mode_objective` through native
   `thread/goal/set`, then verifies it through `thread/goal/get` before product work.
